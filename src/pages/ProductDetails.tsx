...
const ProductDetails = () => {
  ...
  const formatPrice = (price: number, currency: string) =>
    new Intl.NumberFormat('fr-MA', { style: 'currency', currency, minimumFractionDigits: 0 }).format(price);
            ...
            <div>
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex items-center space-x-2">
                  <Button variant="ghost" size="sm" onClick={() => toggleFavorite(product._id)}>
                    <Heart className={`w-4 h-4 ${isFavorite(product._id) ? "fill-current text-red-500" : ""}`} />
                  </Button>
              ...
              <p className="text-lg text-muted-foreground mb-4">{product.brand} • {product.model}</p>
              ...
              <div className="flex flex-wrap gap-3 mb-6">
                <Badge className={getConditionColor(product.condition)}>{t(`product.${product.condition}`)}</Badge>
                <Badge className={getAvailabilityColor(product.availability)}>{t(`product.${product.availability}`)}</Badge>
              ...
              <div className="space-y-3">
                ...
                <Button variant="outline" className="w-full">
                  <ExternalLink className="w-4 h-4 mr-2" /> Compare Prices
                </Button>
              </div>
            </div>
          </div>
              ...
              <CardContent>
                ...
                <div className="mt-6 p-4 bg-muted rounded-lg">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {product.seller.rating && <Badge variant="secondary">⭐ {product.seller.rating}</Badge>}
                  </div>
                </div>
              </CardContent>
            </Card>
